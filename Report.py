...
def TOPResultados(csv_file, words):
    ...
    for row in read:
        ...
        if i > 10:
            break
        addResp(f_append=f, pn = row[1], title = row[4], abs = row[3], date = row[2], words = words, i = i)
        i = i + 1
    f.close()
...
#getReport('./30000000-sort.csv','main','enaex s.a.','plastic bottle',['plastic','bottle'],'1 de junio al 30 de junio')
